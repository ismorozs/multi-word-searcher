...
async function initializePopup(tabId) {
  ...
  _state__WEBPACK_IMPORTED_MODULE_1__["default"].initTabState(tabId);
}
...
function addSearchToContextMenu({ tabId, idx, string }) {
...
async function updateContextMenu () {
  ...
  const newSearchOption = searches.every((s) => s) ? {} : {
    "New...": () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].openSearchGroup()
  }
  ...
  const activeSearchOptions = searches
    .map((s, i) => [
      s,
      {
        Open: () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].openSearchGroup(i),
        Remove: () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].removeSearch(id, i),
      },
    ])
    .filter(([s]) => s);
  ...
  if (activeSearchOptions.length) {
    ...
    activeSearchOptions.push(
      ["separator2", null],
      ["Remove all active", () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].removeAllSearches(id)],
    );
  }
  ...
  const recentSearchesOptions = recentSearches.filter((s) => s).map((s) => [
    s,
    {
      Find: () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].openSearchGroup(undefined, s),
      Remove: () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].removeRecentSearch(s),
    },
  ]);
  ...
  if (recentSearchesOptions.length) {
    recentSearchesSubmenu = {
      ...
      "Remove All Recent": () => _actions__WEBPACK_IMPORTED_MODULE_3__["default"].removeRecentSearch(),
    };
  }
...
function setSearchInTab (tabId, searchId, string) {
  const { tabs, recentSearches } = varstor__WEBPACK_IMPORTED_MODULE_0___default().get();
  tabs[tabId].searches[searchId] = string;
  if (!recentSearches.includes(string)) {
    recentSearches.unshift(string);
  }
  varstor__WEBPACK_IMPORTED_MODULE_0___default().set({ tabs, recentSearches });
}
